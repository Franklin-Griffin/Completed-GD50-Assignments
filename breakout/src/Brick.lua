...
function Brick:init(x, y, lock)
    ...
    self.tier = lock and 20 or 0
    self.color = lock and 12 or 1
...
function Brick:hit()
    ...
    elseif self.lock and self.unlocked then 
        self.psystem:setColors(
        ...
        gSounds['brick-hit-2']:stop()
        gSounds['brick-hit-2']:play()
        -- play a second layer sound
        gSounds['brick-hit-1']:stop()
        gSounds['brick-hit-1']:play()
        gSounds['select']:play()
        self.inPlay = false
    else -- self.lock and not self.unlocked
        self.psystem:setColors(
            paletteColors[3].r / 255,
            paletteColors[3].g / 255,
            paletteColors[3].b / 255,
            100,
            paletteColors[3].r / 255,
            paletteColors[3].g / 255,
            paletteColors[3].b / 255,
...
function Brick:render()
    if self.inPlay then
        if not self.lock then
            ...
            love.graphics.draw(gTextures['main'], 
                gFrames['bricks'][self.unlocked and 21 or 22], self.x, self.y)
        end
    end
end
