...
function PlayState:enter(params)
    ...
    self.scoreGoal = self.level * 5000
end
...
function PlayState:update(dt)
    ...
    -- if we've clicked, to select or deselect a tile...
    if self.canInput and love.mousePressed then
        -- convert to board x and y
        local mouseX, mouseY = love.mouse.getPosition()
        local gameX, gameY = push:toGame(mouseX, mouseY)
        -- offset board
        gameX = gameX - self.board.x
        gameY = gameY - self.board.y
        local x, y = -1, -1
        for Y = 1, 8 do
            for X = 1, 8 do
                local tile = self.board.tiles[Y][X]
                -- AABB
                if gameX >= tile.x and gameX <= tile.x + 32 and gameY >= tile.y and gameY <= tile.y + 32 then
                    x = tile.gridX
                    y = tile.gridY
                end
            end
            ...
            :finish(function()
                if not self:calculateMatches() then
                    ...
                    :finish(function()
                        self.highlightedTile = nil
                        self.canInput = true
                        if not self.board:isPossible() then
                ...
                elseif not self.board:isPossible() then
                    self.board:initializeTiles()
                    reset = true
                    Timer.after(5, function()
                        reset = false
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        -- remove any tiles that matched from the board, making empty spaces (and add points)
...
function PlayState:render()
    ...
    if self.highlightedTile and self.canInput then
    ...
    if reset then
        love.graphics.printf('BOARD RESET (impossible)', self.board.x, self.board.y + 128, 256, 'center')
    end
end